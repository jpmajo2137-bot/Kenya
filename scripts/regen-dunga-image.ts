/**
 * dunga (찌르다 / to stab) 단어 이미지 재생성 (gpt-image-1)
 * 모든 모드 행에 동일 URL 적용
 *
 * 사용법: npx tsx scripts/regen-dunga-image.ts
 */
...
const WORD = 'dunga'
const SLUG = 'dunga'
const PROMPT = `A simple, clean educational illustration for the verb "to stab" (Swahili vocabulary "dunga") — show only a neutral, safe metaphor: a hand holding a kitchen knife above a wooden cutting board with a vegetable (e.g. tomato), about to chop — cooking context only. No people harmed, no blood, no fighting, no faces, no scary imagery.
