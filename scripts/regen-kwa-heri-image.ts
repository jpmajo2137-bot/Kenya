/**
 * kwa heri (안녕히 가세요 / goodbye) 단어 이미지 재생성 (gpt-image-1)
 * SW/KO 등 모든 모드 행에 동일 URL 적용 → 전체 단어·플래시카드 등 공통 표시
 *
 * 사용법: npx tsx scripts/regen-kwa-heri-image.ts
 */
...
const WORD = 'kwa heri'
const SLUG = 'kwa-heri'
const PROMPT = `A simple, clean educational illustration for "goodbye" / a farewell greeting (Swahili phrase "kwa heri").
Show two friendly people at a simple doorway or path, waving hands to say goodbye — warm, peaceful, everyday scene. No specific real people or logos.
Style: minimalist flat vector illustration, soft bright colors, white or very light background, vocabulary flashcard style.
