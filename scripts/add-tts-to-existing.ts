/**
 * 기존에 TTS 없이 저장된 단어들에 TTS를 추가하는 스크립트
 */
...
async function main() {
  console.log('═══════════════════════════════════════════════════════════')
  console.log('🔊 기존 단어에 TTS 추가 스크립트')
  console.log('═══════════════════════════════════════════════════════════')
  ...
  for (let i = 0; i < wordsWithoutTTS.length; i++) {
    ...
    // 1. 스와힐리어 단어 TTS (KO 모드에서는 word가 스와힐리어)
    if (entry.word) {
      const wordAudio = await synthesizeWithAzure(entry.word, 'sw')
      if (wordAudio) {
        const path = `ko/word/${slugify(entry.word)}-${Date.now()}.mp3`
    ...
    // 2. 한국어 뜻 TTS
    if (entry.meaning_ko) {
      const meaningKoAudio = await synthesizeWithAzure(entry.meaning_ko, 'ko')
      if (meaningKoAudio) {
        const path = `ko/meaning-ko/${slugify(entry.word)}-${Date.now()}.mp3`
        audioUrls.meaning_ko_audio_url = await uploadAudio(meaningKoAudio, path)
        console.log(`   ✅ 한국어 뜻 TTS`)
      }
    }
    ...
    if (entry.meaning_en) {
      ...
      if (meaningEnAudio) {
        const path = `ko/meaning-en/${slugify(entry.word)}-${Date.now()}.mp3`
    ...
    // 4. 예문 TTS (스와힐리어)
    if (entry.example) {
      const exampleAudio = await synthesizeWithAzure(entry.example, 'sw')
      if (exampleAudio) {
        const path = `ko/example/${slugify(entry.word)}-${Date.now()}.mp3`
